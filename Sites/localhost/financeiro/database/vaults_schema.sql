-- Sistema de Vaults com Objetivos
-- Execute após financial_schema.sql

-- Tabela de objetivos dos vaults
...
-- Trigger para atualizar valor_atual do vault quando houver movimentação
DELIMITER $$
...
BEGIN
    ...
    concluido = CASE 
        WHEN valor_atual >= valor_meta THEN TRUE 
        ELSE FALSE 
    END,
    data_conclusao = CASE 
        WHEN valor_atual >= valor_meta AND data_conclusao IS NULL THEN CURDATE()
        WHEN valor_atual < valor_meta THEN NULL
        ELSE data_conclusao
    END
...
-- Inserir dados de exemplo para organização 1
INSERT INTO vault_goals (account_id, titulo, descricao, valor_meta, categoria, cor, icone, prioridade, created_by) VALUES
-- Assumindo que existe um account com tipo 'vault' e id 1
-- Se não existir, criar primeiro uma conta vault
